const popularPlaces = [
  ...
  ['මොරටුව','Moratuwa',6.7730,79.8816],['මීගමුව','Negombo',7.2083,79.8358],['බණ්ඩාරවෙල','Bandarawela',6.8289,80.9914],
...
function friendlyWeatherText({prob, mm, temp, hum, cloud, wind}){
  if (mm >= 8 || prob >= 85) return ['අද හොඳටම වහිනවා. එළියට යනවනම් අනිවාර්යෙන් කුඩයක් අරන් යන්න.', 'තද වැසි', 'var(--danger)', '⛈️'];
  if (mm >= 3 || prob >= 65) return ['අද සැරෙන් වහින්න පුළුවන්. ගමන් බිමන් වලදී ටිකක් සැලකිලිමත් වෙන්න.', 'වැසි වැඩියි', 'var(--danger)', '🌧️'];
  if (mm >= 0.8 || prob >= 40) return ['අද වහින්න පුළුවන්. කුඩයක් ළඟ තියාගත්තොත් හොඳයි.', 'වහින්න පුළුවන්', 'var(--warn)', '🌦️'];
  ...
  if (prob < 25 && mm < 0.3) return ['අද වහින්නේ නැති වෙයි. කාලගුණය සාමාන්‍යයෙන් හොඳයි.', 'වැසි අඩුයි', 'var(--safe)', '🌤️'];
  return ['අද කාලගුණය වෙනස් වෙන්න පුළුවන්. අහස ටිකක් බලලා යන්න.', 'සාමාන්‍යයි', 'var(--safe)', '☁️'];
}
